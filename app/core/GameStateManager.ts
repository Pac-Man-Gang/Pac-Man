...
export function nextGameState(playerDir: Direction): GameState {
    ...
    const pacman = playerDir === undefined
        ? prevGameState.pacman
        : nextPacManState(prevGameState.pacman, playerDir);

    const nextGameState = { ghosts, pacman, score, lives: 0 };

    const eatenGhost = nextGameState.ghosts
        .find((ghost) => ghost.mode === GhostMode.EATEN && prevGameState.ghosts
            .find((g) => g.type === ghost.type)!.mode !== GhostMode.EATEN);
    if (eatenGhost) {
        const ghostSpritePos = calcPixelPos(getGhostSprite(eatenGhost.type).getBoundingClientRect());
        window.dispatchEvent(new CustomEvent<PopupBean>('newPopup', {
            detail: { x: ghostSpritePos.x + 30, y: ghostSpritePos.y - 30, text: '200' }
        }));
        addScore(200);
    };
