...
function makePac(i: number, { minSize, maxSize, minSpeed, maxSpeed }: Required<Omit<Props, 'count' | 'opacity' | 'fps'>>): Pac {
    return {
        key: i,
...
export default function BackgroundPacman({
    ...
    useEffect(() => {
        const id = setInterval(() => {
            setMouthFrame((prev) => {
                ...
                if (opening && prev === 3) { openingRef.current = false; return 2; }
                if (!opening && prev === 1) { openingRef.current = true; return 2; }
    ...
    useEffect(() => {
        const seed = Array.from({ length: count }).map((_, i) =>
            makePac(i, { minSize: minSize!, maxSize: maxSize!, minSpeed: minSpeed!, maxSpeed: maxSpeed! })
        );
        setPacs(seed);
        nextId.current = count;
    }, [count, minSize, maxSize, minSpeed, maxSpeed]);

    const src = useMemo(() => `/assets/pacman/pacman${mouthFrame}.png`, [mouthFrame]);

    const respawn = (deadKey: number) => {
        setPacs((curr) => {
            const filtered = curr.filter(p => p.key !== deadKey);
            const newbie = makePac(nextId.current++, { minSize: minSize!, maxSize: maxSize!, minSpeed: minSpeed!, maxSpeed: maxSpeed! });
    ...
    return (
        ...
        @keyframes fly-right-once {
          ...
          3%   { opacity: ${opacity}; }
          100% { transform: translateX(112vw); opacity: ${opacity}; }
        }
        @keyframes fly-left-once {
          0%   { transform: translateX(112vw); opacity: 0; }
          3%   { opacity: ${opacity}; }
          100% { transform: translateX(-12vw); opacity: ${opacity}; }
        }
      `}</style>
                ...
                return (
                    <div
                        ...
                        style={{
                            ...
                            top: `${p.topVh}vh`,
                            left: 0,
                            transform: p.dir === 'right' ? 'translateX(-12vw)' : 'translateX(112vw)',
                    ...
                    >
                        <img
                            ...
                            width={p.size}
                            height={p.size}
                            style={{ display: 'block', transform: rotation, imageRendering: 'pixelated' }}
                        />
                    </div>
                );
            })}
